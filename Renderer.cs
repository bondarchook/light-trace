...
namespace RayTracer
{
    public class Renderer
    {
        ...
        public Renderer(PictureBox pictureBox, Scene scene)
        {
            ...
            pictureBox.Width = _scene.Width;
            pictureBox.Height = _scene.Height;
            _bitmap = new Bitmap(_scene.Width, _scene.Height);
        ...
        public void Render()
        {
            ...
            var partitions = Partitioner.Create(0, _scene.Width, 80);
        ...
        private void RenderTile(Tuple<int, int> range, ParallelLoopState state)
        {
            ...
            double fovY = _scene.Fov*Math.PI/180.0;
            int height = _scene.Height;
            int width = _scene.Width;
            double hw = width/2.0;
            double hh = height/2.0;

            Bitmap buffer = new Bitmap(range.Item2 - range.Item1, _scene.Height);

            for (int x = range.Item1; x < range.Item2; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    var finalColor = _rayTracer.CalculatePixelColor(x, y, fovY, hw, hh);
